 ...
 * API Get nanj rate
 */ 
exports.nanjRate = async function (currency) {
    if (typeof currency == 'undefined') {
        let valid = {}
        valid.status = 422
        valid.message = "Currency is required."

        return valid
    }

    const nanjServer = new server(appId, secretKey)
    return nanjServer.nanjRate(currency)
}

/**
 * API Get nanj balance
 */
exports.nanjBalance = async function (address) {
    return await generateData.getBalanceNanj(address)
}
